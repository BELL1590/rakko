 ...
 *
 * D5 の変更点:
 * - 白ベースの業務UI（.page-row / .form-section）に整理
 * - 一覧カードに消化率バーと公開URLを載せ、編集・複製・CSV・公開切替を1カードに収める
 * - 編集フォームを「基本情報 / 予約設定 / 予約枠」の3セクションに分割
 * - 枠フォームを「枠の内容 / 定員 / 受付期間・リマインド」の3セクションに分割
 *
...
export function adminPagesListPage(params: {
  ...
  const cards = params.pages
    .map((page) => {
      ...
      const pct =
        page.capacity_total > 0
          ? Math.min(100, Math.round((page.booked_seats / page.capacity_total) * 100))
          : 0;

      return `<article class="page-row${page.status === 'published' ? '' : ' is-draft'}">
  <div class="page-row__head">
    <span class="page-row__title">${esc(page.title)}</span>
    ${statusBadge(page.status)}
  </div>
  <p class="page-row__meta">${esc(PAGE_TYPE_LABEL[page.page_type] ?? page.page_type)}
    ・ 予約枠 ${page.slot_count}件 ・ 作成 ${esc(formatJstIsoLike(page.created_at))}</p>
  <p class="page-row__stat"><strong>${page.booked_seats}</strong><span>/ ${page.capacity_total}名</span></p>
  <div class="progress" aria-hidden="true"><span style="width:${pct}%"></span></div>
  <p class="page-row__url">公開URL：<a href="/reserve/${esc(page.slug)}">${esc(publicUrl)}</a></p>
  <div class="btn-row">
    <a class="btn btn-sm" href="/admin/reservations/${page.id}">編集・予約枠</a>
    ...
    <a class="btn btn-sm btn-secondary" href="/admin/reservations/${page.id}/roster.csv">全枠CSV</a>
...
export function adminPageFormPage(params: {
  ...
  <div class="btn-row">
    <a class="btn btn-sm" href="/admin/slots/${slot.id}">名簿・受付</a>
...
export function slotFormFields(params: {
  ...
  <div class="form-section">
    ...
    <p class="form-note">${when(
      Boolean(slot),
      `既存の確定予約人数（${slot?.booked_seats ?? 0}名）を下回る定員には変更できません。`,
